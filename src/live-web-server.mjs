...
const datahub = new LiveMcpDataHubClient();
  ...
  try {
    if (request.method === 'GET' && url.pathname === '/api/status') {
      return sendJson(response, 200, {
        ...
        catalog: 'local-datahub-core-synthetic-metadata'
      });
    }
    ...
    return sendJson(response, 500, { error: error.message, mode: 'mcp-live' });
  }
});

server.listen(port, '127.0.0.1', () => {
  console.log(`Dollar Data Context Guard live MCP demo: http://127.0.0.1:${port}`);
});
